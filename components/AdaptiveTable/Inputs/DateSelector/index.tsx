...
// import {FaCalendarAlt} from '@/components/FaIcons/icons';
import {FilterType, RangeDate} from '@/client/pagination/IPagerParams';
...
export default function DateSelector(props: IInputProps) {
    ...
    const DATE_RANGE_SELECTOR_ID = useMemo(
        () => `DateRangeSelectorId_${name}`,
        [name],
    );
    ...
    const makeRange = useCallback(
        (day, range) =>
            DateUtils.isDayInRange(day, range)
                ? range
    ...
    const getInitialRangeState = useCallback(() => {
        if (isSingleDate) {
            ...
            return makeFromTo(
                from ? moment(from).toDate() : undefined,
                to ? moment(to).toDate() : undefined,
            );
        }
    }, [isSingleDate, makeFromTo, value]);

    const [selectedDays, setSelectedDays] = useState(
        makeFromTo(undefined, undefined),
    );
    ...
    const startDayChangeHandler = useCallback(
        day => {
    ...
    const endDayChangeHandler = useCallback(
        day => {
            const firstDayRange = makeFromTo(
                selectedDays.from,
                selectedDays.from,
            );
    ...
    const windowClickDateRange = useCallback(
        (event: any) => {
            // eslint-disable-next-line @typescript-eslint/no-unused-expressions
            showCalendars &&
                //@ts-ignore
                !clickOutSideTheBlock(event, DATE_RANGE_SELECTOR_ID) &&
    ...
    const placeholder = useMemo(
        () => (isSingleDate ? t('select-date') : t('select-date-range')),
        [isSingleDate, t],
    );
