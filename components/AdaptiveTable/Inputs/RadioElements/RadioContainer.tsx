...
import {IOptions, FilterType} from '@/client/pagination/IPagerParams';
...
export default function RadioContainer(props: IRadioContainerProps) {
    ...
    useEffect(() => {
        ...
        if (items && items.length > 0) {
            const findedOption = items?.filter(
                (option: IOptions) => option?.value === value,
            );
            initValue = (
                findedOption?.length > 0 ? findedOption[0] : items[0]
            ) as IOptions;
        }
    ...
    return (
        <>
            {optionsRender?.length > 0 && (
                <div className={`flex ${directionStyle}`}>{optionsRender}</div>
            )}
        </>
    );
}
