...
import { addEntities, deleteEntities } from "@/client/store/actions";
...
function User({ id }: { id: number }) {
	useEffect(() => {
		toast(`card No-${id} update`);
	}, [id]);
	const user = useSelector<AppState, IUser>(
		(state) => state.entities.users[id] as IUser
	);
	...
	return (
		<div className="flex flex-col w-1/2 border">
			...
			<Button
				onClick={() => dispatch(addEntities({ users: { [id]: updatedUser } }))}
