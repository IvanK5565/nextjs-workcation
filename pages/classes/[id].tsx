...
export default function Home({
	...
	return (
		<>
			<div>
				<p className="pl-10">id:{_class.id}</p>
				<form onSubmit={handleSubmit} className="flex flex-col w-min pl-10">
					<input
						type="hidden"
						name="class_id"
						className="block"
						defaultValue={_class.id}
					/>
					<input
						type="text"
						name="title"
						className=""
						defaultValue={_class.title}
						required
					/>
					<select
						name="teacher_id"
						id="teacher_id"
						defaultValue={_class.teacher_id}
						required
					>
