      ...
      <div className="xl:max-w-6xl lg:mx-auto">
        <div className="lg:w-7/12 lg:pl-12 lg:pr-8">
          ...
          <div className="mt-8 lg:absolute lg:inset-y-0 lg:right-0 lg:w-5/12">
            <img className="h-56 w-full object-cover object-center rounded-lg shadow-xl lg:inset-0 lg:h-full lg:shadow-none lg:rounded-none" src="/images/beach-work.jpg" alt="woman workcationing on beach" />
            <svg className="hidden lg:block absolute inset-y-0 left-0 w-16 h-full" viewBox="0 0 100 100" preserveAspectRatio="none" fill="none" xmlns="http://www.w3.org/2000/svg">
              <polygon className="text-gray-100" fill="currentColor" points="0,0 0,100 100,0" />
            </svg>
          </div>
          <SignIn className={landing == Landing.Login ? "block" : "hidden"} onRegister={() => setLanding(Landing.Register)} />
          <Register className={landing == Landing.Register ? "block" : "hidden"} onLogin={() => setLanding(Landing.Login)} />
          <About className={landing == Landing.About ? "block" : "hidden"} onLogin={() => setLanding(Landing.Login)} />
        </div>
      </div>
    </div>
  </div>
