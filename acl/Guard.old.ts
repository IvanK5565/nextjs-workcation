...
class Guard {
    ...
    public allow(
        ...
        secret = secret ?? this.mIdentity?.secret ?? null;
    ...
    private init(acl: Acl, secret?: string) {
        const s = secret ? secret + ":" : "";
        for (const role in this.mRoles) {
            if (this.mRoles.hasOwnProperty(role)) {
                ...
                let parentRole: string[]|null = null;// Modified by Ivan
                if (parentRoleId !== null) {
                    parentRole = [];
                    // parentRoleId.forEach((pRole) => parentRole.push(s + pRole));
                    parentRole = parentRoleId.map((pRole) => (s + pRole)) // Modified by Ivan
                }
                // const pr: string[] | string =
                const pr: string[] | string|null = // Modified by Ivan
                    !!secret && role === ROLE.GUEST
                        ? ROLE.GUEST
                        : parentRole;
